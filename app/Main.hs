...
actionDecision index playerList foodList| isBlueZone wx wy    = let [vx, vy] = [(0x8000 - wy),(0x8000 - wx)]
  ...
  where pl = fromJust . fromJust $ lookup index playerList
        [wx, wy] = [playerWorldXf pl, playerWorldYf pl]
        ep = searchEnemy pl . catMaybes . M.elems . M.fromList $ filter ((/= index) . fst) playerList
...
aimFood pl foodList | isBlueZone wx wy = Nothing
  ...
  where f  = fromMaybe (FoodBlock []) (lookup (toEnum . fromEnum $ by `shiftL` 8 + bx) foodList)
